...
# Modified to run across 30 random seeds
...
if __name__ == '__main__':
    ...
    # Define transforms (these can remain constant)
    image_size = 192
    ...
    for seed_id in range(total_seeds):
        ...
        # 2. Re-initialize Dataloaders (to ensure shuffling is determined by the new seed)
        ...
        # 3. Re-initialize Model (Fresh weights)
        ...
        # 4. Re-initialize Criterion and Optimizer
        criterion = nn.CrossEntropyLoss()
        optimizer = optim.Adam(model.parameters())
        
        # Define train and test functions inside loop or pass model/optim as args
        # (Redefined here to capture the specific model instance for this seed)
        ...
        # 5. Set up Directories for this specific SEED
        TEST_ID = f'TEST_ID001_SEED_{seed_id}' # Dynamic Naming
        ...
        n_epoch = 30
        print(f'Training for {len(range(n_epoch))} epochs with Seed {seed_id}\n')
        ...
        print(f'Training for Additional {len(range(n_epoch_additional))} epochs with Seed {seed_id}\n')
