...
MODELS_CONFIG = {
    'TEST_ID0025': {'label': 'Base Model', 'color': 'gray',      'style': '--'}, # مدل پایه
    'TEST_ID0028': {'label': 'TRL Model',  'color': 'blue',      'style': '-.'}, # مدل TRL
    'TEST_ID0032': {'label': 'Our Method', 'color': 'red',       'style': '-'}   # متد ما
}

# --- تنظیمات دقیق Tiny ImageNet (طبق درخواست شما) ---
NUM_SEEDS = 16
TOTAL_EPOCHS = 15
UNFREEZE_EPOCH = 5
...
# ==========================================
# 3. تابع رسم نمودار مقایسه‌ای (با قابلیت Log)
# ==========================================
def plot_comparison(metric_key, range_type, scale_type):
    config = METRICS_CONFIG[metric_key]
    
    fig, ax = plt.subplots(figsize=(10, 6))
    # گرید بندی (which='both' برای خطوط ریز لگاریتمی)
    ...
    # --- ذخیره سازی ---
    # ساختار: .../tiny_image_net/Comparison/{Scale_Type}/{Range_Type}/filename.png
...
if __name__ == "__main__":
    print(f"--- Generating Comparison Plots (Tiny ImageNet - Linear & Log) ---")
    print(f"Output Root: {BASE_OUTPUT_DIR}")
    print(f"Settings: Seeds={NUM_SEEDS}, Epochs={TOTAL_EPOCHS}, Unfreeze={UNFREEZE_EPOCH}\n")
    
    for metric_name in METRICS_CONFIG.keys():
        print(f"Processing Metric: {metric_name}...")
        
        for r_type in ['Full_Range', 'Unfrozen_Only']:
            # حلقه جدید برای Linear و Log
