...
# توابع
# ==========================================
def get_train_loss(folder_name):
    """
    این تابع داده‌های Train Loss را می‌خواند.
    """
    ...
    try:
        with open(file_path, 'r') as f:
            content = f.read()
            # --- تغییر کلیدی: جستجو برای Train epoch و مقدار loss ---
            # الگو: Train epoch 8: ... loss=0.00269...
...
if __name__ == "__main__":
    ...
    all_seeds_data = []
    print(f"Processing {NUM_SEEDS} seeds for {SEED_PREFIX} (TRAIN LOSS)...")

    # جمع‌آوری داده‌ها
    for seed in range(NUM_SEEDS):
        folder_name = f'{SEED_PREFIX}_SEED_{seed}'
        # فراخوانی تابع جدید برای دریافت Train Loss
        losses = get_train_loss(folder_name)
    ...
    if len(all_seeds_data) > 0:
        ...
        ax.set_title(f'Train Loss Base Model Tiny Image Net Dataset', fontsize=14, fontweight='bold')
        ax.set_xlabel('Epochs', fontsize=12)
        ax.set_ylabel('Train Loss', fontsize=12)
        ax.legend(loc='upper right') # لجند بالا سمت راست
        ...
        # نام فایل خروجی
        save_path = 'images/train_loss_test_id_0025.png'
