...
The power of tensor regression has been addressed in many prior papers \cite{r17}\cite{r18} \cite{r19} \cite{r20}. However, most of these works, usually rely on analytical solution and large tensors. In \cite{r33}, a non-linear Tensor-Train decomposition is used instead of tucker decomposition. While many works focuses on individual layers, in \cite{r34}, T-Nets attempts to provide a single high-order, low-rank tensor that jointly capture the full structure of a neural network. 
...
Tensor unfolding, also known as matricization, converts a higher-order tensor into a matrix or a vector. Let $\mathcal{T} \in \mathbb{R}^{I_1 \times I_2 \times \ldots \times I_N}$ be an $N$-order tensor. In our case, tensor folding is the inverse operation of tensor unfolding, which reconstructs a higher-order tensor from a matrix or vector. 
...
\begin{figure}
	\centering
	\includegraphics[scale=0.03]{fig1.png}
...
\label{subsubsec:tcl}
\textit{Tensor Contraction Layer} (TCL) \cite{r1} is a form of feature extraction and could also be used for embedding. The prime idea behind TCL is Tucker decomposition; Via multiplying matrices to each mode of the input tensor, we can obtain a denser tensor (Appendix \ref{A-tcl}). Figure \ref{fig:2-3} demonstrate the TCL process on a 3-order tensor. 
...
\begin{figure}
	\centering
	\includegraphics[scale=0.025]{fig2-3.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=0.02]{fig5.png}
	\caption{
...
\begin{figure}
	\centering
	\includegraphics[scale=0.03]{fig6-7-8.png}
	\caption{
...
\label{sec:experiments}
	...
	We conducted a series of experiments to assess the performance of our proposed framework.Our experiments were performed on GeForce RTX 4090;
	Datasetes MNIST, CIFAR10, Tiny ImageNet-200 applied to ResNet50, ResNet101, VGG-19, and classic CNN demonstrate that our methods can significantly reduce the number of parameters while maintaining or even improving classification accuracy. In Tables
	\ref{table:resnet_mnist_cifar10}
	,
	\ref{table:vgg_tiny}
	, and 
	\ref{table:cnn_mnist}
	parameters include only the classifier parameters. For a better representation of our methods, we ignored adaptive average pooling layers in all models. \(^A\) and \(^C\) are splitting approaches.
	
	\begin{table}
		\centering
		\caption{
		\textbf{Performance evaluation of ResNet50 and ResNet101}, Our methods reduce the number of parameters by more than 90 \% while achieving near similar accuracy. More detailed Table(s) (Appendix \ref{more-table-1}).
			}
	...
	\begin{table}
		...
		\caption{
			\textbf{Tiny ImageNet-200 performance on VGG-19},
			 Given extreme number of classifier parameters for a VGG-19 model, we applied TCL layers before TRL layers. More detailed Table(s) (Appendix \ref{more-table-2}). 
			}
	...
	\begin{table}
		\centering
		\caption{
			\textbf{Classic CNN-MNIST},  Our methods are dependent on a good convolutional foundation. More detailed Table(s) (Appendix \ref{more-table-3}). 
			}
...
%	\subsection{Analysis}
%	
	Our framework performs exceptionally well compared to traditional TCL TRL and baseline models across various datasets and architectures. Notably, our approach significantly reduces the number of parameters without acutely compromising accuracy . In figure \ref{fig:9}, loss and accuracy of selected models and datasets are plotted. \textbf{Ablation study} is carried out in the Appendix \ref{ablation-study}.
	
	\begin{figure}
		\centering
		\includegraphics[width=1.0\textwidth]{plot.png}
		\caption{Accuracy and loss plots (Baseline, TCL TRL, Our Method), more detailed plots (Appendix  \ref{more-plots-1})}
		\label{fig:9}
	\end{figure}
	
\section{Conclusion}
\label{sec:conclusion}
Deep neural networks, especially CNNs, often work with multi-dimensional tensors. Despite their empirical success, these architectures usually contain one or more fully connected layers which not only destroy the natural multi-linear structure of the data, but also over parameterize the model. By introducing tensorized layers such TRL and TCL we can achieve near similar accuracy while preserving the multi-modal structure of the data and reducing the number of classifier parameters; However, these methods perform poorly when the approximated rank is too low. We introduced SATURN, a framework which maintains the quality of the network and reduce the number of classifier parameter by more than 96 \%. Our experiment, evaluated on CIFAR10, MNIST, and Tiny ImageNet-200, applied to RestNet50, RestNet101, VGG19 , and classic CNNs demonstrate that by splitting the input tensor via an approach that does not disturb the multi-linear structure of the data, a TCL or TRL with fewer parameters can be performed which also maintains the accuracy. Based on our observations, many popular CNNs  learn their convolutional layers based on the flattening operation and their following FC layers. Therefore, for our framework to achieve best accuracy, the models need to be trained completely for a few iterations. While our framework and similar approaches offer theoretical advantages, they currently do not exhibit the expected speed. This limitation underscores the primary challenge in applying tensor methods to models, as the development of optimal implementations remains a pending task. 
...
\begin{align}
	\label{eq:tcl}
	&\left(V^{(1)},\dots,V^{(N)}\right)_{1:N} \mathcal{X} = \mathcal{X}_{[0]} (V^{(1)}\otimes\dots\otimes V^{(N)})^T
\end{align}
...
and $\mathcal{G} \in \mathbb{R}^{I_1\times\dots\times I_N\times O}$.
...
Therefore, with ideal ranks, $\forall i \hspace{2mm}\hat{R}_i \leq R_i$ and $\sum_{i=0}^{N} r_i\times L_i \leq \alpha$, inequality \ref{eq:proof-1-14} holds.
\end{proof}

Given no ideal, or some ideal ranks, there is no guarantee that inequality \ref{eq:proof-1-14} holds without further constraints. We will briefly discuss some ideal settings for semi ideal ranks and splits.  
...
Given no ideal, or some ideal ranks, there is no guarantee that inequality \ref{eq:proof-2-29} holds without further constraints. We will briefly discuss some ideal settings for semi ideal ranks and splits.  
...
\label{subspace-tcl}
%We discuss the subspace attribute of Our method on TCL, classic TCL, and MLP. Given input tensor $\mathcal{X} \in \mathbb{R}^{S_0\times I_1\times\dots\times I_N}$, $\mathcal{X}_{[0]}$ is the vectorization of all samples within batch $S_0$.
%
%Consider:
%
%\begin{align}
%	\label{eq:subspace-tcl-42}
...
%\begin{align}
%	\label{eq:subspace-tcl-42}
%	\mathcal{Y} = \bar{\mathcal{X}}\times V^T \hspace{10mm}V\in \mathbb{R}^{R\times I}\hspace{2mm},\hspace{2mm}R = R_1R_2\dots R_N 
%\end{align}
%
%Consider space $\Phi$ such that:
%
%\begin{align}
%	\label{eq:subspace-tcl-43}
%	\Phi = \left\{V | V \in \mathbb{R}^{R\times I}\right\}
%\end{align}
%
%Assuming $V^{(i)} \in \mathbb{R}^{R_i\times I_i}$, then given a TCL with factors $V^{(i)}$:
%
%\begin{align}
%	\label{eq:subspace-tcl-44}
%	\hat{\mathcal{X}} = \mathcal{X} \times_1 V^{(1)}\times_2 V^{(2)}\dots \times_{N+1} V^{(N)}
%\end{align}
%
%Then we can consider the vectorized version as :
%
%\begin{align}
%	\label{eq:subspace-tcl-45}
%	\mathcal{Y} = \bar{\mathcal{X}}\times \hat{V}^T \hspace{10mm} \hat{V} = \left(V^{(1)}\otimes V^{(2)}\otimes \dots\otimes V^{(N)} \right) \in \mathbb{R}^{R\times I}
%\end{align}
%
%Consider space $\Psi$ such that:
%
%\begin{align}
%	\label{eq:subspace-tcl-46}
%	\Psi = \left\{V = V^{(1)}\otimes V^{(2)}\otimes \dots\otimes V^{(N)} | \forall i \hspace{2mm} V^{(i)} \in \mathbb{R}^{R_i\times I_i}\right\}
%\end{align}


We discuss the subspace attribute of Our method on TCL, classic TCL, and MLP. Given input tensor $\mathcal{X} \in \mathbb{R}^{S_0\times I_1\times\dots\times I_N}$, $\mathcal{X}_{[0]}$ is the vectorization of all samples within the batch $S_0$.
...
Assuming $V^{(i)} \in \mathbb{R}^{R_i\times I_i}$, then a TCL with factors $V^{(i)}$ can be defined as:

\begin{align}
	\label{eq:subspace-tcl-47}
	\hat{\mathcal{X}} &= \left(V^{(1)},\dots,V^{(N)}\right)_{1:N} \mathcal{X}
	\nonumber\\
	&= \bar{\mathcal{X}} \times (\hat{V})^T 
\end{align}
...
\begin{align}
	...
	\Psi = \left\{\bar{\mathcal{X}} \hat{V}^T | \hat{V} = V^{(1)}\otimes V^{(2)}\otimes \dots\otimes V^{(N)}  \in \mathbb{R}^{R\times I} \land \forall i \hspace{2mm} V^{(i)} \in \mathbb{R}^{R_i\times I_i}\right\}
\end{align}

It is clear that $\Psi \subseteq \Phi$. So the solution of a TCL is a subset of standard MLP but with less parameters. 
...
\begin{align}
	...
	\hat{\mathcal{X}} &= \left(U^{(1)},\dots,U^{(N)}, W^{(1)},\dots,W^{(N)}\right)_{1:2N} \tilde{\mathcal{X}}
	\nonumber \\
	& = \left(U^{(1)} \otimes W^{(1)} ,\dots,U^{(N)} \otimes W^{(N)}\right)_{1:N} \Gamma^{-1}(\tilde{\mathcal{X}})
	\nonumber\\
	& = \left(V^{(1)} ,\dots,V^{(N)}\right)_{1:N} \mathcal{X}
	\nonumber\\
	&= \bar{\mathcal{X}} \times (\tilde{V})^T 
\end{align}

where $\Gamma^{-1}$ is the inverse of the reshape operation; $\tilde{V} = \left(V^{(1)}\otimes V^{(2)}\otimes \dots\otimes V^{(N)} \right) \in \mathbb{R}^{R^\prime\times I}$, $V^{(i)} = U^{(i)} \otimes W^{(i)}$, and $R^\prime = \prod_{i=1}^{N} R_i r_i$
...
\begin{align}
	...
	\tilde{V} = V^{(1)}\otimes V^{(2)}\otimes \dots\otimes V^{(N)}  \in \mathbb{R}^{R^\prime\times I} \right\}
	\nonumber \\
	 &\forall i \hspace{2mm} V^{(i)} = U^{(i)} \otimes W^{(i)} \in \mathbb{R}^{R_ir_i\times I_i} 
	 \nonumber \\
	 &\forall i \hspace{2mm} U^{(i)} \in \mathbb{R}^{R_i \times \frac{I_i}{L_i}}, W^{(i)} \in \mathbb{R}^{r_i\times L_i}
\end{align}


It is clear that $\Omega \subseteq \Psi \subseteq \Phi$. So the solution of a TCL on our framework is a subset of standard TCL and standard MLP but with less parameters. 
...
Additional experiments evaluated on MNIST, CIFAR10, and Tiny ImageNet-200 applied to ResNet50, ResNet101, VGG-19 , and classic CNN. In out experiments, to better represent the effect of our methods, we ignored adaptive average pooling layer of all models. 
...
We generated synthetic samples from a Gaussian distribution $\mathcal{N}(0, 3)$ such that $\mathcal{X} \in \mathbb{R}^{S\times32}$, where $S$ is the number of samples. Given a fixed $\mathcal{W} \in \mathbb{R}^{32\times32}$, we consider $\mathcal{Y} = \mathcal{X} \times \mathcal{W}^T$. Note that $\mathcal{X}$ is the vectorization of each samples if they were higher order tensors and $\mathcal{Y} \in \mathbb{R}^{S\times 32}$. Our goal is to estimate $\mathcal{W}$ using our modules trained on noisy $\mathcal{X} + \epsilon$, where $\epsilon$ is from a Gaussian distribution $\mathcal{N}(0,1)$. We evaluated our framework, standard TRL, and standard FC using RMSE; This study is carried out with a single layer model trained on noisy data using Adam optimizer. In figure \ref{fig:ablation}, a comparison of different methods using same ranks is provided. 

\begin{figure}
	\centering
	\includegraphics[scale=0.04]{ablation.png}
	\caption{
		TODO
	}
	\label{fig:ablation}
