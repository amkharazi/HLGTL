...
%\dots
\textit{Fully Connected} (FC) layers are often used in many neural network architectures such as \textit{Convolutional neural networks} (CNNs), Transformers, and many others. The data is flattened before being fed to the FC layers which not only changes the topological structure but also requires many parameters; \textit{Tensor Contraction Layer} (TCL) and \textit{Tensor Regression Layer} (TRL) are capable candidates to use instead of FC layers which preserve the multilinear structure of data as well as reduce the cost of execution. To exploit the capability of TRL and TCL even further, Our method uses specific splitting approaches  to generate thinner higher dimensional tensors from the original data to reduce the cost by a greater factor while also preserving the accuracy. Our experiments on CIFAR10 and MNIST, applied to known models such as ResNet, VGG, and other typical CNNs demonstrate our method's ability to reduce the cost compared to previous methods while also maintaining the accuracy. 
